...
- 历史会话搜索已接入完整本地链路：按会话标题和全部消息内容过滤，显示命中数量、命中类型、内容片段、空状态和清空入口。
- 会话标题固定使用该会话的第一个用户问题命名，后续追问只更新时间和消息内容，不再覆盖历史列表标题。
- 左下角账户菜单已优化为可关闭的账户弹层：点击头像打开，点击页面空白处或按 `Esc` 自动收起，菜单内保留账户摘要和退出系统操作。
- 历史会话删除菜单已与账户弹层统一交互：点击更多按钮打开，左键点击菜单/更多按钮/用户卡片以外的任意窗体区域或按 `Esc` 自动收起；外部点击使用捕获阶段监听，避免被页面内部事件拦截，视觉样式保持同一套轻量弹层。
- 推荐问题按钮已改为纯文字胶囊样式，不再显示前置彩色图标。
- 助手回答已改为主流 AI 聊天式富文本排版：Markdown 标题、加粗、列表、行内代码和代码块会被渲染为结构化样式，不再裸露 `###`、`**` 等标记。
- 对话消息已补齐聊天头像：系统消息使用纯白底项目透明 logo 作为助手头像，用户消息使用用户头像图标，左右镜像布局更接近真实聊天界面。
- 复制回答已支持 Clipboard API 失败时的降级复制，并会显示复制成功/失败提示。
- 重新生成按钮已限制为只在当前会话最新一条助手回答上显示；历史助手回答只保留复制，避免旧回答触发上下文不明确的重试。
- 来源引用组件已支持默认展示 Top 3，超过 3 条时可展开全部。
- 管理员问答页已挂载到 `AdminShell`，最左侧 `AdminNav` 支持折叠/展开；展开和折叠时都使用贴左边缘的同尺寸浮动按钮，折叠后左栏彻底收起。
...
- 管理员最左侧应用导航支持折叠/展开；展开和折叠时都使用贴左边缘的同尺寸浮动按钮，折叠后左栏彻底收起，状态保存在 `localStorage`
- 管理员最左侧应用导航只显示图标和中文名称，不显示 QA / DOC / LOG 英文缩写
- 普通用户问答工作区 `/chat`
...
- 历史侧栏菜单状态 helper `src/components/app/historySidebarMenus.ts`
- 问答工作区组件 `src/components/chat/ChatWorkspace.vue`
- 输入框组件 `src/components/chat/ChatComposer.vue`
- 回答卡片组件 `src/components/chat/AnswerCard.vue`，支持用户/系统头像、Markdown 回答正文、来源文档、复制和最新回答重新生成操作
- 重新生成显隐规则 `src/chat/retryVisibility.ts`
...
- `HistorySidebar`：承载历史搜索、会话切换、会话删除菜单、账户菜单和统一点击外部关闭逻辑
- `RecommendedQuestions`
- `AnswerCard`：承载用户/系统头像、消息气泡、Markdown 回答正文、来源文档、复制和最新回答重新生成操作
- `SourceReferences`
- `FeedbackBar`
...
- 推荐问题以纯文字胶囊按钮展示，可以点击后直接发送。
- 输入框 placeholder 使用宽泛提示“请输入你的问题”，左下角以轻量键帽样式展示快捷键说明：Enter 发送，Shift + Enter 换行。
- 左侧历史会话可以点击切换右侧对应会话页，不会重复发送历史问题。
- 左侧历史会话标题使用会话内第一条用户问题，连续追问不会把标题改成最后一个问题。
- 搜索历史会话可以按标题和消息内容命中，列表会显示结果数量、命中来源、内容片段，并支持一键清空搜索。
- 返回后展示 `answer`、`answer_type`、`confidence`、`references`。
- `answer_type = refused` 时展示拒答状态，不伪造来源。
- 只有当前会话最后一条助手回答显示“重新生成”，历史助手回答只显示“复制”。
...
- 回答类型和置信度不放在顶部问题标题下，统一放入回答底部的“来源文档”卡片头部。
- 回答正文使用 `MarkdownAnswer` 渲染安全 Markdown 子集，覆盖标题、段落、列表、加粗、行内代码和代码块；不使用 `v-html` 直接注入模型返回内容。
- 支持默认展示 Top 3，超过 3 条时可展开全部。
- 补齐复制、基础反馈和最新回答重试入口；历史回答不显示重试入口。
...
- 展开和折叠时都使用贴左边缘的同尺寸浮动按钮；折叠后左栏彻底收起。
- 导航项只显示图标和中文名称，不显示 QA / DOC / LOG 英文缩写。
...
- 左侧历史会话的显示时间不再直接读取本地缓存中的 `time` 文本，而是由 `updatedAt` 时间戳动态格式化生成。
- 新建会话、发送问题、回答完成或出错时，`updatedAt` 均由 `Date.now()` 写入，来源是浏览器所在设备的真实当前时间。
- 历史分组同样基于 `updatedAt` 动态计算为“今天 / 昨天 / 更早”，刷新页面或跨天后不会继续信任旧的缓存分组文本。
- 当前会话历史仍保存在 `localStorage`。后续接入真实 `GET /api/sessions` 时，应将后端返回的创建/更新时间统一映射为 `createdAt` 和 `updatedAt`，继续由前端格式化显示。
